...
const ColumnVisibilityDropdown = ({ columns, hiddenCols, onToggle }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-1.5 w-48 rounded-[10px] border border-hairline bg-surface shadow-lg py-1.5">
          ...
          {columns.map((col) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-2 text-[13px] text-ink hover:bg-surface-1 transition-colors cursor-pointer"
              >
                <span>{col}</span>
                <span className={`w-4 h-4 rounded-[4px] border flex items-center justify-center transition-colors ${visible ? "bg-[var(--module-ticket)] border-[var(--module-ticket)]" : "border-hairline bg-surface"}`}>
