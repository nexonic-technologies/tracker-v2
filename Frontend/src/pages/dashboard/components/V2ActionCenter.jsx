...
export default function V2ActionCenter({ items = [], layoutVariant = 'manager', refresh }) {
  ...
  if (!items || items.length === 0) {
    return (
      <section className="bg-surface rounded-2xl shadow-xs border border-hairline-soft p-6 text-center py-10 flex flex-col items-center justify-center">
        <div className="p-3 rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400 mb-3">
          <Check className="h-6 w-6 stroke-[2.5]" />
        </div>
        <h4 className="text-sm font-bold text-ink">All Action Items Cleared</h4>
        <p className="text-xs text-ink-muted mt-1 max-w-xs">
          No pending leave approvals, overdue tasks, or escalated tickets require your attention right now.
        </p>
      </section>
    );
  }
