...
export default function AssetAllocationDetailPage() {
  const { id } = useParams();
  ...
  const fetchRecord = useCallback(async () => {
    ...
    try {
      setError(null);
      const res = await readDetailed("assets_allocations", {
        id,
        populateFields: {
      ...
      if (res?.data) {
        ...
        setError("Asset allocation record not found.");
      }
    } catch (err) {
  ...
  const handleAction = async (approve) => {
    ...
    try {
      ...
      const successMsg = approve ? "Asset allocation approved successfully!" : "Asset allocation rejected.";
      await update("assets_allocations", data._id, {
        status: targetStatus,
        remarks: comment,
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60dvh] bg-canvas" data-module="assets">
        <div className="flex flex-col items-center gap-3">
          <div className="h-8 w-8 border-2 border-[var(--module-accent)] border-t-transparent rounded-full animate-spin" />
          <p className="text-sm text-ink-muted">Loading asset allocation details...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="lmx-content py-8" data-module="assets">
        <button
          onClick={() => navigate(-1)}
        ...
        <div className="p-6 bg-surface border border-hairline rounded-tracker-card flex flex-col items-center gap-4 text-center max-w-lg mx-auto shadow-xs">
          ...
          <div>
            <h2 className="text-lg font-bold text-ink">Allocation Not Found</h2>
            <p className="text-sm text-ink-muted mt-1">{error || "The asset allocation could not be loaded."}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="lmx-content py-6" data-module="assets">
      <div className="mb-6 flex items-center justify-between">
        ...
        <span className="text-xs text-ink-subtle">
          Allocation ID: <code className="font-mono text-ink">{data._id}</code>
        </span>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="tracker-card p-6 bg-surface space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-hairline-soft pb-4">
              <div>
                <span className="lmx-page-eyebrow">Asset Management · Allocation Review</span>
                <h1 className="text-xl font-bold text-ink mt-1">Review Asset Allocation</h1>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-xs font-semibold text-ink-subtle uppercase tracking-wider">Allocated To Employee</h3>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-base font-bold text-ink mt-0.5">
                    {allocationDateStr} {expectedReturnStr ? `— ${expectedReturnStr}` : " (Permanent)"}
                  </p>
                </div>
              </div>

              {/* Asset Specific Card */}
              <div className="col-span-1 md:col-span-2 bg-canvas-muted p-4 rounded-tracker-md border border-hairline space-y-3">
                <h4 className="text-xs font-bold text-ink-muted uppercase tracking-wider border-b border-hairline pb-1.5">Asset Specifications</h4>
            ...
            {isPending ? (
              <div className="space-y-4">
                <p className="text-xs text-ink-muted leading-relaxed">
                  Review the asset specifications and allocated personnel before approving or rejecting this allocation.
                </p>
                ...
                <div className="flex flex-col gap-3 pt-2">
                  ...
                  >
                    <Check size={18} /> Approve Asset Allocation
                  </button>
                  <button
              ...
              <div className="flex flex-col items-center justify-center py-6 text-center gap-3">
                <div className={`h-12 w-12 rounded-full flex items-center justify-center ${
                  data.status === "Active" || data.status === "Approved" ? "bg-tracker-success-light text-tracker-success" : "bg-tracker-danger-light text-tracker-danger"
                }`}>
                  {data.status === "Active" || data.status === "Approved" ? <Check size={24} /> : <X size={24} />}
                </div>
                <div>
