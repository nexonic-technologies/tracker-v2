...
import {
  ...
  FolderKanban, CheckSquare, Layers, User, Search, Sparkles, SlidersHorizontal,
  FileSpreadsheet, ArrowUpRight, CheckCircle2, Clock, ShieldCheck
} from 'lucide-react';
...
const COLOR_TOKENS = {
  ...
  'Approved': { bg: '#ecfdf5', text: '#059669', border: '#a7f3d0', dot: '#10b981' },
  'In Progress': { bg: '#eff6ff', text: '#2563eb', border: '#bfdbfe', dot: '#3b82f6' },
  'In Review': { bg: '#faf5ff', text: '#9333ea', border: '#e9d5ff', dot: '#a855f7' },
  'To Do': { bg: '#fffbeb', text: '#d97706', border: '#fde68a', dot: '#f59e0b' },
  'Backlogs': { bg: '#f8fafc', text: '#64748b', border: '#e2e8f0', dot: '#94a3b8' },
  ...
  'Weekly Priority': { bg: '#faf5ff', text: '#9333ea', border: '#e9d5ff', dot: '#a855f7' },
};

const PALETTE = ['#0ea5e9', '#8b5cf6', '#10b981', '#f59e0b', '#ec4899', '#6366f1', '#14b8a6', '#f97316'];

const REPORT_OPTIONS = [
  { value: 'status', label: 'By Status', icon: 'BarChart3', description: 'Grouped by workflow status' },
  { value: 'priority', label: 'By Priority', icon: 'TrendingUp', description: 'Grouped by priority level' },
  { value: 'client', label: 'By Client', icon: 'PieChart', description: 'Grouped by client organization' },
  { value: 'assignee', label: 'By Assignee', icon: 'User', description: 'Workload breakdown per member' },
  { value: 'taskType', label: 'By Task Type', icon: 'Filter', description: 'Grouped by task classification' },
  { value: 'projectType', label: 'By Project Type', icon: 'Layers', description: 'Grouped by project category' },
  { value: 'monthly', label: 'Monthly Trend', icon: 'Calendar', description: 'Historical monthly distribution' },
  { value: 'completion', label: 'Completion Rate', icon: 'CheckSquare', description: 'Finished vs pending summary' }
];
  ...
  return (
    <div className="relative inline-flex items-center justify-center shrink-0">
      ...
      <div className="absolute flex flex-col items-center text-center">
        ...
        <span className="text-[11px] font-semibold text-ink-subtle mt-0.5">Total Tasks</span>
      </div>
    </div>
  );
};
...
const TaskReports = () => {
  ...
  const getReportConfig = (type) => {
    const configs = {
      ...
      priority: { type: 'summary', groupBy: 'priorityLevel' },
      client: { type: 'summary', groupBy: 'clientId', populate: ['clientId'] },
      assignee: { type: 'summary', groupBy: 'assignedTo', populate: ['assignedTo'] },
      taskType: { type: 'summary', groupBy: 'taskTypeId', populate: ['taskTypeId'] },
      projectType: { type: 'summary', groupBy: 'projectTypeId', populate: ['projectTypeId'] },
      monthly: { type: 'summary', groupBy: 'createdAt', dateGrouping: 'month' },
      completion: { type: 'summary', groupBy: 'status', filter: { status: { $in: ['Completed', 'Approved'] } } }
    };
  ...
  const totalTasks = useMemo(() => safeReportData.reduce((sum, item) => sum + (item.count || 0), 0), [safeReportData]);
  const avgTasksPerGroup = useMemo(() => totalTasks / (safeReportData.length || 1), [totalTasks, safeReportData]);
  const completedTasks = useMemo(() =>
    safeReportData.filter(item => ['Completed', 'Approved'].includes(item._id || item.name))
  ...
  const handleExportCSV = () => {
    ...
    const currentOption = REPORT_OPTIONS.find(o => o.value === reportType);
    ...
    link.href = url;
    link.setAttribute('download', `task_report_${reportType}_${dateRange.startDate}_to_${dateRange.endDate}.csv`);
  ...
  /* Dynamic Token Resolution with Status Master fallback */
      ...
      return (
        <div className="flex items-center gap-3 min-w-[180px]">
          ...
          <div className="flex-1 h-2 bg-surface-1 rounded-full overflow-hidden border border-hairline-soft">
            <div
              className="h-full rounded-full transition-all duration-500 bg-[var(--module-project)]"
  ...
  return (
    <div className="flex flex-col h-full bg-canvas p-4 sm:p-6 space-y-5 overflow-y-auto" data-module="project">

      {/* ── Page Header ── */}
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-tracker-lg bg-[var(--module-project-light)] border border-[var(--module-project-light)] flex items-center justify-center text-[var(--module-project)] shadow-sm">
            <FolderKanban size={20} />
          </div>
          <div>
            <p className="lmx-page-eyebrow mb-0.5">PROJECT MANAGEMENT</p>
      ...
      <div className="tracker-card-plain p-4 space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-2 pb-2 border-b border-hairline-soft">
          <div className="flex items-center gap-2">
            <SlidersHorizontal size={15} className="text-[var(--module-project)]" />
        ...
        {lastUpdated && (
          <div className="flex items-center justify-between pt-2 border-t border-hairline-soft text-[11px] text-ink-subtle">
            <span>Analyzing <strong>{totalTasks}</strong> tasks across <strong>{safeReportData.length}</strong> categories</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div>
              <div className="flex items-center justify-between pb-3 mb-3 border-b border-hairline-soft">
                <h3 className="text-[14px] font-semibold text-ink flex items-center gap-2">
                  <BarChart3 size={16} className="text-[var(--module-project)]" />
          ...
          <div className="tracker-card-plain p-4 flex flex-col items-center justify-center text-center">
            <h3 className="text-[14px] font-semibold text-ink self-start mb-3 flex items-center gap-2">
              <PieChart size={16} className="text-[var(--module-project)]" />
              Proportion Share
            </h3>
            <DonutChartWidget
      ...
      <div className="tracker-card-plain overflow-hidden flex-1">
        <div className="p-4 border-b border-hairline flex items-center justify-between flex-wrap gap-2 bg-surface-1/40">
          <div className="flex items-center gap-2">
            <FileSpreadsheet size={16} className="text-[var(--module-project)]" />
            <h2 className="text-[15px] font-semibold text-ink">
              Detailed Data Table — {activeOption.label}
            </h2>
            <span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-[var(--module-project-light)] text-[var(--module-project)]">
